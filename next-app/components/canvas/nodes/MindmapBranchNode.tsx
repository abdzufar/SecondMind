...
export function MindmapBranchNode({ data }: NodeProps<MindmapNode>) {
  const hasPendingTasks = useCanvasStore((s) =>
    s.todos.some((t) => !t.isCompleted && t.taskText === data.label)
  );

  return (
    <div className={`xf-branch${data.isActive ? " is-active" : ""}${data.isSelected ? " is-selected" : ""}${hasPendingTasks ? " has-pending-tasks" : ""}`}>
