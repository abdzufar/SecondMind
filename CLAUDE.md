...
- [~] Halaman login (Google + email/password) — backend partner udah kelar sisi mereka: `lib/auth.ts` (Google OAuth + Credentials provider, auto-register user Google baru, session JWT ada `user.id`), `POST /api/auth/register`, `types/next-auth.d.ts` — semua sudah nyata, bukan lagi placeholder. **Yang baru dikerjakan di sisi frontend**: `components/AuthProvider.tsx` (client wrapper tipis buat `next-auth/react`'s `SessionProvider`) dipasang di `app/layout.tsx` — prasyarat mutlak sebelum `useSession()`/`signIn()` bisa dipakai di halaman manapun, dan sekarang sudah terpasang serta diverifikasi (`/api/auth/session` balik `200 {}` pas belum login). `app/login/page.tsx` dan `app/register/page.tsx` sendiri **masih persis placeholder** — `handleSubmit` cuma `router.push`, tidak ada tombol Google, tidak ada pemanggilan `signIn`/`fetch` ke `/api/auth/register` sungguhan. Itu langkah berikutnya, belum dikerjakan.
