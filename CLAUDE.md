...
- [~] Halaman login (Google + email/password) — backend partner udah kelar sisi mereka: `lib/auth.ts` (Google OAuth + Credentials provider, auto-register user Google baru, session JWT ada `user.id`), `POST /api/auth/register`, `types/next-auth.d.ts` — semua sudah nyata, bukan lagi placeholder. Frontend: `components/AuthProvider.tsx` (client wrapper tipis buat `next-auth/react`'s `SessionProvider`) dipasang di `app/layout.tsx` — prasyarat sebelum `useSession()`/`signIn()` bisa dipakai, sudah terpasang & diverifikasi (`/api/auth/session` balik `200 {}` pas belum login). `app/register/page.tsx` **sudah wired beneran** — `handleSubmit` baca `FormData`, validasi password vs konfirmasi di klien, `fetch POST /api/auth/register`, banner error (`.form-error` di `auth.css`) buat password gak cocok atau email sudah terdaftar (409), sukses → redirect ke `/login`. Diverifikasi lewat Playwright (mismatch, sukses, duplicate email — tiga-tiganya benar). **`app/login/page.tsx` masih persis placeholder** — `handleSubmit` cuma `router.push`, tidak ada tombol Google, tidak ada pemanggilan `signIn` sungguhan. Itu langkah berikutnya.
