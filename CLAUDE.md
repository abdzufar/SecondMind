...
- [~] Ganti isi `lib/api.ts` dari mock ke fetch asli — dikerjakan satu fungsi per satu, bukan sekaligus.
  - `generateMindmap()` **sudah** manggil `POST /api/mindmap/generate` beneran (`FormData`: topic/timeframe/verbosity/language/file, sesuai kontrak §6). Hasilnya dibaca lewat `WireMindmap`, terus di-bridge juga ke `mindmapsDb` lokal (`mindmapsDb.push`) — supaya `getMindmaps()` yang **masih mock** (dashboard "Riwayat") tetap nemuin record hasil generate asli. Bridge ini dicabut pas `getMindmaps()` ikut dipindah ke fetch asli.
  - `getMindmap(id)` **sudah** manggil `GET /api/mindmap/:id` beneran (gak lagi butuh bridge, baca langsung dari MongoDB). `app/canvas/[id]/page.tsx` juga dapat state error baru (`loadError` + `.canvas-loading--error`) — sebelum ini, kegagalan `getMindmap` cuma `console.error` dan macet selamanya di "Memuat canvas…"; sekarang tampil pesan "Mindmap tidak ditemukan atau gagal dimuat." + link balik ke composer. Ini jadi perlu karena dengan data asli, 404 itu skenario nyata (bukan cuma teoretis kayak pas masih mock).
  - Fungsi lain (`elaborateNode`, `getMindmaps`, `saveMindmap`, `deleteMindmap`, todo) masih mock, nunggu giliran masing-masing.
