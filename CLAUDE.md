...
**Keputusan yang masih menggantung:** nasib "tandai selesai" per-node (§9 M4) — tetap visual lokal, atau digeser ke status To-Do (M8) yang persisted. Belum ada urgensi mendesak buat mutusin ini.
...
- **M7 sisa**: `elaborateNode()` (tombol "Pecah jadi sub-cabang" di drawer) masih mock — beda fitur dari chat (yang udah wired ke `POST /api/chat` beneran), nunggu kontrak endpoint elaborate final di backend.
